...
# DevOps Agent — DefenseFlow Policy Editor
...
| All drivers fail to compile | New `DefenseProDriver` method — must implement in all 13 versions |
| `Could not find artifact` | `./mvnw clean install -DskipTests` from root |
| Frontend `MODULE_NOT_FOUND` | `cd ui/policy-app-<ver> && npm start` first (builds `dist/`) |
...
- Dependency versions live in `policy-bom/pom.xml` — never in child modules
- Each `ui/policy-app-<version>/` is independent — never run npm from `ui/` root
